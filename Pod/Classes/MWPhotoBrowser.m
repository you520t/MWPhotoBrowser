...
- (void)updateNavigation {
    ...
    if (_gridController) {
        ...
        _pageInfoLable.hidden = NO;
    } else {
        pageMessage = nil;
        _pageInfoLable.hidden = YES;
    }
...
-(void)saveImage:(id)sender{
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        {
            //无权限
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"没有访问相册权限，请在设置中打开" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil, nil];
            [alert show];
        }
        else
        {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    if (error == nil) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"保存成功" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil, nil];
        [alert show];
    }else{
